...
/**
 * DirectMessages — the direct-messaging experience, moved unchanged from the
 * Messages page. Reports the current unread count up via onUnreadCount so a
 * parent page can show one combined unread badge.
 */
export default function DirectMessages({ onUnreadCount }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const unread = (data.messages || []).filter(
        (m) => !m.read && m.recipient_email === (data.profile || {}).user_email
      ).length;
  ...
  const markAsRead = async (message) => {
    if (!message.read && message.recipient_email === profile?.user_email) {
      try {
        ...
        onUnreadCount?.(messages.filter(m => !m.read && m.id !== message.id && m.recipient_email === profile?.user_email).length);
      } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">Direct messaging with teachers and students</p>
        </div>
