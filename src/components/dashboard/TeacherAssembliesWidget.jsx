...
export default function TeacherAssembliesWidget() {
  ...
  const next = assemblies.find(a => new Date(a.start_time) >= new Date() && a.status !== 'cancelled');
  if (!next) return null;
    ...
    <Card className="surface-card card-hover">
      <CardContent className="pt-6">
        <div className="flex items-center gap-2 mb-3">
          ...
          <div><p className="text-sm font-semibold text-foreground">Next Assembly</p><p className="text-xs text-muted-foreground">Relevant to your classes</p></div>
        </div>
