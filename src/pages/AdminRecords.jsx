...
import { Search, ChevronRight, PenLine, Shield, Trophy, Sparkles, HardDrive } from 'lucide-react';
              ...
              {filtered.length === 0 ? (
                ...
                <TableRow key={r.id} className={`hover:bg-slate-50 ${r.status === 'awaiting_admin_signature' ? 'bg-orange-50/30' : r.status === 'approved' ? 'bg-green-50/30' : ''}`}>
                  ...
                  <TableCell>
                    ...
                    {r.verify_id && (
                      <p className="text-xs text-slate-400 mt-0.5 font-mono truncate max-w-[100px]">{r.verify_id}</p>
                    )}
                  </TableCell>
