...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { class_id, date, marks } = body;
    ...
    for (const m of marks) {
      ...
      const existing = await svc.entities.AttendanceRecord.filter({
        class_id, student_email: m.student_email, date, school_id: actor.school_id,
      });
      const payload = {
      ...
      if (existing[0]) {
        await svc.entities.AttendanceRecord.update(existing[0].id, payload);
      } else {
    ...
    return new Response(JSON.stringify({ saved }), { status: 200, headers: cors });
  } catch (e) {
