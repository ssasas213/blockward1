  ...
  try {
    ...
    return Response.json({
      ...
      student: {
        ...
        // Extended customisation + content presets (validated at write time).
        avatar_frame: profile.avatar_frame || 'none',
        card_style: profile.card_style || 'elevated',
        corner_style: profile.corner_style || 'rounded',
        surface_pattern: profile.surface_pattern || 'none',
        hero_style: profile.hero_style || 'banner',
        accent_gradient: profile.accent_gradient || null,
        card_density: profile.card_density || 'comfortable',
        highlight_style: profile.highlight_style || 'carousel',
        forced_scheme: profile.forced_scheme || 'auto',
        ...
        custom_links: Array.isArray(profile.custom_links) ? profile.custom_links.slice(0, 4) : [],
        section_order: Array.isArray(profile.section_order) ? profile.section_order : [],
