// ============================================================================
// achievementRequestFlow — the single implementation of the student achievement
// request lifecycle: submit (with anti-abuse caps), withdraw, republish,
// verifier sign-off, admin second approval, Tier 3 / independent external
// verification via one-time links, request changes, reject, and minting to the
// canonical credential pipeline (credentialDelivery).
//
// Used by BOTH the achievementRequestAction endpoint (user-authenticated) and
// the demo seeder (service-role with synthetic actors), so the demo world is
// produced through the exact same transitions, notifications and delivery code
// the real product runs. No flow logic is duplicated anywhere.
//
// Every function returns { status, payload }; the endpoint wraps them in
// Response.json. ctx = { ip, country } — derived from the HTTP request by the
// endpoint, or a synthetic context by the seeder.
// ============================================================================
import {
...
} from './achievementRequests.ts';
import { mintRequestCredential } from './credentialDelivery.ts';
import {
...
} from './independentVerification.ts';
import { MAX_TEAM_PARTICIPANTS } from './teamCredentials.ts';
...
const bad = (error, status = 400) => ({ status, payload: { ok: false, error } });
...
export function buildSignoff(body, actor, email, actorName, role, country, now) {
  const signoff = {
    method: body.method,
...
async function normalizeIndependentForm(svc, actor, form) {
  ...
  else if (!REQUEST_CATEGORIES.includes(category)) errors.push('Invalid category');
...
export async function normalizeForm(svc, actor, form) {
  ...
  if (category && !REQUEST_CATEGORIES.includes(category)) errors.push('Invalid category');
...
export async function submitRequest(svc, actor, body, ctx) {
  const action = body.action; // save_draft | submit | resubmit
...
export async function withdrawRequest(svc, actor, body) {
  const email = actor.actor_email;
  if (actor.actor_role !== 'student') return bad('Only students can withdraw their requests', 403);
  let request = null;
  try {
...
export async function retryMint(svc, actor, body) {
  const email = actor.actor_email;
  if (actor.actor_role !== 'student') return bad('Only students can republish their requests', 403);
  let request = null;
  try {
    const rows = await svc.entities.AchievementRequest.filter({ id: body.request_id || 'none' });
    request = rows?.[0] || null;
...
export async function runReviewerAction(svc, actor, body, ctx) {
  ...
  if (action === 'sign' || action === 'bulk_sign') {
    ...
    return ok(200, { ok: failed.length === 0, signed, failed });
  }
  ...
  if (action === 'admin_approve') {
    ...
    if (!mint.ok) {
      ...
      return bad('Signed off, but publishing failed: ' + mint.error, 500);
    }
    await notifyEvent(svc, {
      to_email: request.student_email,
      school_id: request.school_id,
  ...
  if (action === 'request_changes') {
    ...
    return ok(200, { ok: true, status: 'changes_requested' });
  }
  ...
  if (action === 'reject') {
    ...
    if (!reason) return bad('A reason is required when rejecting');
    await svc.entities.AchievementRequest.update(request.id, {
      status: 'rejected',
...
// ═══════════════════ EXTERNAL VERIFIER (token-authenticated) ═══════════════════

export async function runExternalAction(svc, body, ctx) {
  const token = body.token;
  if (!token) {
    return bad('This link is invalid — check you opened the exact link from the email.');
  }
  ...
  if (request.status === 'withdrawn') {
    return bad('The student withdrew this request before you reviewed it. No action is needed.');
  }
  if (request.status !== 'awaiting_external_verification') {
    return bad('This verification has already been completed or is no longer awaiting your confirmation.');
  }
  if (request.external_token_expires_at && new Date(request.external_token_expires_at) < new Date()) {
    return bad('This verification link has expired. Ask the student to send a fresh verification request.');
  }
  ...
  // Verifier reports the claim as false — the request is rejected and the
  ...
  if (isIndependent) {
    ...
    let accountEmails = [];
    try {
      const prows = await svc.entities.UserProfile.filter({ user_email: request.student_email });
      const p = prows[0];
  ...
  if (isIndependent) {
    try {
      flags = await independentVerifierFlags(svc, request, email, ctx?.ip || null);
  ...
  if (!mint.ok) {
    ...
    return bad('Something went wrong publishing this — the student has been notified. Your confirmation was recorded; nothing more is needed from you.', 500);
  }
