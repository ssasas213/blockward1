...
export async function logRoleGrant(svc, grant) {
  ...
  try {
    await svc.entities.AuditLog.create({
      ...
      action: 'role_changed',
      target_type: 'user',
      target_id: grant.record_id || null,
      target_email: grant.granted_to_email,
      target_label: grant.granted_to_name || grant.granted_to_email,
      before_summary: grant.old_role ? `role: ${grant.old_role}` : 'role: (new account)',
      after_summary: `role: ${grant.role}`,
